...
logger = get_logger(__name__, domain_file="aq_forecast.log")
...
async def fetch_city_aq(
    ...
    variables = ["pm2_5", "european_aqi_pm2_5"]
    params = {
        ...
        try:
            async with session.get(url, params=params, timeout=120) as response:
                ...
                for entry in data:
                    ...
                    city_data = {
                        var: hourly.get(var, []) for var in variables + ["time"]
                    }
...
def extract_aq_forecast():
    target_date = datetime.now().strftime("%Y-%m-%d")
    logger.info(f"===== START AQ FORECAST CRAWL {target_date} =====")

    cities_file = (
        Path(__file__).resolve().parent.parent.parent / "data/dim_locations.csv"
    )
    if not cities_file.exists():
        logger.error("❌ No city location found.")
    ...
    if df.empty:
        ...
        raise ValueError("No data crawled for air quality forecast")
    ...
    logger.info(f"===== FINISHED AQ FORECAST CRAWL {target_date} =====\n")
