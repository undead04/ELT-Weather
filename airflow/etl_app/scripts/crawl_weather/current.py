...
async def crawl_all_cities(cities: pd.DataFrame):
    ...
    # Split cities into chunks of 30
    city_chunks = [
...
def extract_weather_current():
    target_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(f"===== START WEATHER CURRENT CRAWL {target_date} =====")

    base_dir = Path(__file__).resolve().parent.parent.parent
    province_path = base_dir / "data/dim_locations.csv"

    if not province_path.exists():
        raise FileNotFoundError(f"No province file found at {province_path}")
    ...
    if df.empty:
        ...
        raise ValueError("No data crawled for current weather")
    ...
    logger.info(f"===== FINISHED WEATHER CURRENT CRAWL {target_date} =====\n")
